...
public class Perlin {
    ...
    public double Get(double x, double y, double z) {
        ...
        double x1 = MoreMath.LerpUnclamped(Gradual(aaa, xf, yf, zf), // The gradient function calculates the dot product between a pseudorandom
            Gradual(baa, xf - 1, yf, zf), // gradient vector and the vector from the input coordinate to the 8
            u);
        double x2 = MoreMath.LerpUnclamped(
            Gradual(aba, xf, yf - 1,
            ...
            u);
        double y1 = MoreMath.LerpUnclamped(x1, x2, v);

        x1 = MoreMath.LerpUnclamped(Gradual(aab, xf, yf, zf - 1), Gradual(bab, xf - 1, yf, zf - 1), u);
        x2 = MoreMath.LerpUnclamped(Gradual(abb, xf, yf - 1, zf - 1), Gradual(bbb, xf - 1, yf - 1, zf - 1), u);
        double y2 = MoreMath.LerpUnclamped(x1, x2, v);

        return (MoreMath.LerpUnclamped(y1, y2, w) + 1) / 2; // For convenience we bound it to 0 - 1 (theoretical min/max before is -1 - 1)
    }
    ...
    private static double Gradual(int hash, double x, double y, double z) {
        int h = hash & 0b1111; // Take the hashed value and take the first 4 bits of it
        ...
        return ((h & 1) == 0 ? u : -u) +
            ((h & 2) == 0 ? v : -v); // Use the last 2 bits to decide if u and v are positive or negative.
