...
namespace PER.Demo {
    public class Game : IGame {
        ...
        public void Setup() {
            ...
            testProgressBar = new ProgressBar(renderer) {
                position = new Vector2Int(0, 58),
                size = new Vector2Int(80, 2)
            };
            _ui.Add(testProgressBar);
        }
...
cf000000FFbFF0000FFcthanks for watchingcfFFFFFFFFb00000000c everyone, uhit like, subscribe, good luck, bbye!!".Split('\n'),
            ...
            if(testProgressBar is not null &&
               (renderer.input?.mousePosition.InBounds(testProgressBar.bounds) ?? false) &&
               renderer.input.MouseButtonPressed(MouseButton.Left)) {
                testProgressBar.value = renderer.input.normalizedMousePosition.x;
            }
